...
function GenerateInvoice() {
  ...
  const validateForm = () => {
    const requiredFields = [
      ...
      "billingCity", "billingState", "billingPincode", "billingStateCode",
      "shippingName", "shippingAddress", "shippingCity", "shippingState",
      "shippingPincode", "shippingStateCode", "placeOfDelivery", "orderNo",
      "orderDate", "invoiceNo", "invoiceDate"
    ];

    for (const field of requiredFields) {
      if (!invoiceData[field]) {
        setError(`${field.replace(/([A-Z])/g, ' $1').replace(/^./, function(str){ return str.toUpperCase(); })} is required.`);
        return false;
      }
    }
    ...
    for (const item of invoiceData.items) {
      if (!item.description || item.unitPrice <= 0 || item.quantity <= 0) {
        setError("All item fields (description, unit price, and quantity) are required and must be valid.");
        return false;
      }
    }
      ...
      >
        ...
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-300 pt-2">
            Please Enter image size of 1 MB
          </p>
        </div>
